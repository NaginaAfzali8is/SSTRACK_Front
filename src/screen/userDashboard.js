...
function UserDashboard() {
    ...
    const { data: ownerData, error: ownerError, isLoading: isOwnerLoading } = useQuery({
        ...
        staleTime: 20000, // 5 minutes (data is considered fresh for 5 minutes)
        cacheTime: 20000, // 15 minutes (data stays in cache for 15 minutes)
        initialData: () => {
            // Fetch initial data from cache if available
            return queryClient.getQueryData(['ownerData', ownerUrl]);
        },
    ...
    useEffect(() => {
        // setLoading(true)
        if (ownerData) {
            setData2(processData(ownerData));
        }
    }, [ownerData]);
    ...
    useEffect(() => {
        // setLoading(true)
        if (managerData) {
            setData2(processData(managerData));
        }
    }, [managerData]);
    ...
    const processData = (data) => {
        ...
        return [...onlineUsers, ...offlineUsers].filter(user => !user.isArchived && !user.UserStatus);
    };



    const updateSocketData = (prevData, data) => {
        return prevData.map(user => {
            if (user.userId === data.user_id) {
                const lastActiveTime = new Date(data.createdAt);
                const timeAgo = getTimeAgo(lastActiveTime);
                return { ...user, recentScreenshot: { ...user.recentScreenshot, key: data.key }, minutesAgo: timeAgo };
            }
            return user;
        });
    };
    ...
    const renderSkeleton = () => (
        <div className="dashsheadings">
            <div className="companyNameverified">
                <Skeleton circle={true} height={50} width={50} />
                <h5 className="dashCompanyName"><Skeleton width={100} /></h5>
            </div>
            <div className="companyNameverified lastActive" style={{ width: "100%" }}>
                <Skeleton height={75} width={75} />
                <p className="dashheadingtop"><Skeleton width={150} /></p>
            </div>
            <div className="nameVerified">
                <p className="dashheadingtop textalign"><Skeleton width={50} /></p>
                <p className="screenShotAmount"><Skeleton width={50} /></p>
            </div>
            <div className="nameVerified">
                <p className="dashheadingtop textalign"><Skeleton width={50} /></p>
                <p className="screenShotAmount"><Skeleton width={50} /></p>
